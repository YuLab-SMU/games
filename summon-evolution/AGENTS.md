...
4. 背景处理（二选一，新增主题优先"切割后置透明"）：
   - 生成时让 gpt 用统一纯色背景（如浅蓝 `#B5E2FC`），切割后用脚本把背景置透明（与 `stripBg` 同一套容差逻辑：diff<35 全透明、35–70 渐变）；
   - 或保留纯色背景，由游戏加载图片后用 JS `stripBg` 去除。
   - 现状：**minecraft classic 的 17 张 PNG 已直接在文件里置透明**，皮肤选择器缩略图（裸 `<img>`）不再露底；JS `stripBg` 保留作兜底（对已透明图幂等，无需移除）。
5. 代码接入：
